...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Evaluate complex prediction performance.")
    parser.add_argument('--pred_path', type=str, help='Path to predicted PPI file.')
    parser.add_argument('--complex_graph_path', type=str, help='Path to complex graphs file.')
    args = parser.parse_args()

    # Read the PPI data
    ppis = read_ppis(args.pred_path)
    # Load the complex graphs
    complex_graphs = pickle.load(open(args.complex_graph_path, 'rb'))
    ...
    for graph in tqdm(complex_graphs):
        ...
        gt_num_components = nx.number_connected_components(graph)
        num_components = nx.number_connected_components(pred_sub_graph)
        if gt_num_components == 1:
            if num_components == 1:
